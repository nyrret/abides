...
cdef class OrderPyx:
    ...
    def __init__(self, agent_id, time_placed, str symbol, quantity, is_buy_order, order_id=None, tag=None):
        ...
        #global Order_order_ids
        #if Order_order_ids is None:
        #    Order_order_ids = set()
        Order_order_ids.add(self.order_id)
    ...
    cdef generateOrderId(self):
        ...
        global Order_order_id
        global Order_order_ids
        #if Order_order_ids.find(Order_order_id) == Order_order_ids.end():
        if not Order_order_id in Order_order_ids:
            oid = Order_order_id
        else:
            Order_order_id += 1
